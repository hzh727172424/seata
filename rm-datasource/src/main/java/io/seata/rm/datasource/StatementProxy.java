...
public class StatementProxy<T extends Statement> extends AbstractStatementProxy<T> {
  ...
  /*
    以更新为例说明这个代码逻辑
    1.(statement, args) -> statement.executeUpdate((String) args[0]) 传入的是一个接口 StatementCallback
    StatementCallback默认就是调用父类的statement的excuteUpdate的方法也就是执行sql
    ExecuteTemplate是seata定义的执行模板方法。这个模板中有执行业务sql前的beforeImage和afterImage
   */
    @Override
